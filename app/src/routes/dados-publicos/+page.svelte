<script lang="ts">
	/**
	 * PUBLIC DASHBOARD - Anonymous health data transparency page
	 *
	 * CRITICAL SECURITY:
	 * - NO student names, IDs, or any PII displayed
	 * - NO links to individual student pages
	...
	const yearTriggerContent = $derived(
		yearOptions.find((option) => option.value === selectedYear.toString())?.label ||
			'Selecione o ano'
	);

	// Handle year change - navigate to public route
	async function handleYearChange(newYear: string) {
		...
		// Navigate to public dashboard URL with year parameter
		await goto(`/dados-publicos?ano=${newYear}`, {
			replaceState: true,
			noScroll: true
		});
...
<svelte:head>
	<title>Dados Públicos - PSE Atibaia</title>
	<meta
		name="description"
		content="Transparência dos dados de saúde escolar do PSE Atibaia. Dados agregados e anonimizados garantindo a privacidade dos alunos."
	/>
</svelte:head>

<div class="container mx-auto p-8 md:p-6">
	<!-- Public Page Header with Transparency Message -->
	<div class="mb-8 rounded-lg border border-blue-200 bg-blue-50 p-6">
		<div class="flex items-start space-x-4">
			...
			<div class="flex-1">
				<h1 class="text-2xl font-bold text-blue-900">Transparência - PSE Atibaia</h1>
				<p class="mt-2 text-blue-800">
					Dashboard público de dados de saúde escolar do Programa Saúde na Escola de Atibaia
				</p>
				<p class="mt-3 text-sm text-blue-700">
					<strong>Privacidade garantida:</strong> Os dados apresentados são agregados e anonimizados,
					garantindo a privacidade dos alunos. Nenhuma informação que possa identificar um aluno individualmente
	...
	<div class="mb-8 flex items-center justify-between">
		<div>
			<h2 class="text-xl font-semibold text-gray-900">
				Dados de Saúde - Ano {data.currentYear}
			</h2>
			<p class="text-sm text-gray-600">Estatísticas agregadas das avaliações realizadas</p>
		</div>
			...
			<Card.Header>
				...
				<Card.Description>
					Porcentagem de alunos avaliados por tipo de avaliação (Total: {data.coverageStats
						.total_students} alunos avaliados)
				</Card.Description>
			</Card.Header>
			<Card.Content>
				<div class="space-y-6">
					...
					<div>
						<div class="mb-2 flex items-center justify-between">
							...
							>
								{data.coverageStats.anthropometric_completed} / {data.coverageStats.total_students}
								({data.coverageStats.anthropometric_percentage.toFixed(1)}%)
							</span>
						</div>
	...
	<!-- Charts Grid - Desktop First Responsive -->
	...
	<footer class="mt-12 border-t border-gray-200 pt-6">
		<div class="text-center text-sm text-gray-600">
			<p class="mb-2">
				<strong>Sobre a Privacidade dos Dados:</strong>
			</p>
			<p class="mb-4">
				Todos os dados apresentados nesta página são agregados e anonimizados. Não é possível
				identificar alunos individualmente a partir das informações exibidas. Os dados servem
				exclusivamente para fins de transparência pública sobre o Programa Saúde na Escola.
			</p>
			<p class="text-xs text-gray-500">
				© 2025 Secretaria Municipal de Saúde - PSE Atibaia
			</p>
		</div>
	</footer>
</div>
